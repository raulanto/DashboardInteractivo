<template>
    <div :style="panelStyle" class="panel-flotante absolute" :class="[
        ...
        <UCard :ui="{
            ...
            <template #header>
                <div class="flex justify-between items-start cursor-grab active:cursor-grabbing select-none"
                    ...
                    <div class="flex gap-1">
                        <UTooltip text="Duplicar panel">
                            <UButton icon="i-heroicons-document-duplicate" size="xs" color="gray" variant="ghost"
            ...
            <div class="p-4 overflow-auto" :style="{ height: `calc(${panel.tamaño.height}px - 80px)` }">
                <!-- Estadística -->
                <div v-if="panel.tipo === 'estadistica'" class="flex flex-col items-center justify-center h-full">
                    ...
                    <UBadge :label="panel.data.subtitulo" size="md" color="primary" variant="subtle" />
                </div>
                ...
                <div v-else-if="panel.tipo === 'lista'" class="space-y-2">
                    <UCard v-for="item in panel.data.items" :key="item.id" :ui="{ body: { padding: 'p-2' } }">
                        <div class="flex items-center gap-3">
                            <UAvatar :text="item.id.toString()" size="sm" color="primary" />
                            ...
                            <UBadge :label="item.estado" size="xs"
                                :color="item.estado === 'Completado' ? 'success' : item.estado === 'En progreso' ? 'error' : 'primary'"
                                variant="subtle" />
                        </div>
                    </UCard>
                </div>
                ...
                <div v-else-if="panel.tipo === 'tabla'" class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                        <thead class="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                <th v-for="col in panel.data.columnas" :key="col"
                        ...
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                            <tr v-for="(fila, idx) in panel.data.filas" :key="idx"
                                class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                <td v-for="(celda, cellIdx) in fila" :key="cellIdx"
                                    class="px-3 py-2 text-sm text-gray-900 dark:text-gray-100 whitespace-nowrap">
                                    <UBadge v-if="cellIdx === 2" :label="celda" size="xs"
                                        :color="celda === 'Activo' ? 'primary' : 'secondary'" variant="subtle" />
                ...
                <div v-else-if="panel.tipo === 'notas'" class="h-full">
                    <UTextarea placeholder="Escribe tus notas aquí..." :rows="10" autoresize variant="none"
                        :ui="{ base: 'resize-none' }" />
                </div>
                ...
                <div v-else class="h-full flex flex-col items-center justify-center text-center">
                    ...
                    <UBadge :label="`Tipo: ${panel.tipo}`" size="sm" color="primary" variant="subtle" />
                </div>
            </div>
        </UCard>

        <!-- Handle para redimensionar -->
        <div class="resize-handle absolute bottom-1 right-1 w-5 h-5 cursor-nwse-resize opacity-50 hover:opacity-100 transition-opacity rounded-sm bg-primary/20 flex items-center justify-center"
